// core/sovereign-brain.js — BSFM ULTIMATE OPTIMIZED PRODUCTION BRAIN v2.8.4 (UNSTOPPABLE FUNDING FIXED)
// 🔥 CRITICAL FIX: Re-enabled mandatory conditional USDC → ETH self-funding check.
// 🔥 FINAL FIX: Implemented explicit function signature access in getFallbackQuote to resolve Ethers.js v6 ambiguity and 'could not decode result data' error.
...
class ProductionSovereignCore extends EventEmitter {
    ...
    async fundWalletWithUsdcSwap(amountUsdc = 5.17) {
        ...
        try {
            ...
            try {
                ...
                // Fallback: Use direct call with corrected ABI and explicit signature
            ...
            if (receipt.status === 1) {
                // IMPORTANT: The actual amountOut is not returned directly by the swap function itself,
                // but we rely on the quote (amountOut) for logging purposes as a target.
                const ethReceived = ethers.formatEther(amountOut); 
                this.logger.info(`🎉 USDC→ETH SWAP SUCCESS! Target Received: ${ethReceived} ETH (based on quote)`);
    ...
    // Fallback quote method - NOW STABLE
    async getFallbackQuote(amountIn, poolFee) {
        try {
            // CRITICAL FIX: Explicitly access the single-return overload signature 
            // and use .staticCall to stabilize the Ethers v6 view call,
            // which resolves the 'could not decode result data' error.
            const quoteFn = this.quoter['quoteExactInputSingle(address,address,uint256,uint24,uint160)'];

            if (!quoteFn) {
                 throw new Error("Fallback quote function signature not accessible.");
            }
            
            // Execute the static call. The result is a single BigNumber (amountOut).
            ...
            // FIX: The single-return overload returns a BigNumber, not an array.
            // We package it into the expected 4-item tuple for the calling function.
            return [amountOut, 0n, 0n, 0n]; 
        } catch (fallbackError) {
            this.logger.error(`❌ Fallback quote also failed (Final attempt, contract logic likely failed): ${fallbackError.message}`);
            return null;
        }
    }
    ...
    async initialize() {
        this.logger.info('🧠 Initializing ULTIMATE OPTIMIZED PRODUCTION BRAIN v2.8.4 (UNSTOPPABLE FUNDING FIXED)...');
