...
class EnhancedRevenueEngine {
    ...
    // 🎯 OPTIMIZED: BWAEZI to WETH Swap at 1:100 USDC rate
    async executeBwaeziToWethSwap(bwaeziAmount) {
        try {
            const logger = getGlobalLogger('BwaeziSwap');
            
            // BWAEZI token ABI with approve and transfer functions
            const bwaeziABI = [
                {
                    "constant": false,
                    "inputs": [
            ...
            const bwaeziContract = new this.blockchain.web3.eth.Contract(
                bwaeziABI, 
                BWAEZI_TOKEN_CONFIG.CONTRACT_ADDRESS
            );

            // Check BWAEZI balance first
            const bwaeziBalance = await bwaeziContract.methods.balanceOf(this.transactionManager.account.address).call();
            const readableBalance = this.blockchain.web3.utils.fromWei(bwaeziBalance.toString(), 'ether');
            
            logger.info(`💰 BWAEZI Balance: ${readableBalance} tokens`);
            
            if (parseFloat(readableBalance) < bwaeziAmount) {
                throw new Error(`Insufficient BWAEZI balance: ${readableBalance} available, ${bwaeziAmount} requested`);
            }

            // Convert BWAEZI amount to wei
            const bwaeziAmountWei = this.blockchain.web3.utils.toWei(bwaeziAmount.toString(), 'ether');
            
            // 🎯 STEP 1: Approve Uniswap to spend BWAEZI tokens
            logger.info('🔐 Approving BWAEZI tokens for swap...');
            
            const approveTxData = {
                to: BWAEZI_TOKEN_CONFIG.CONTRACT_ADDRESS,
                data: bwaeziContract.methods.approve(
                    LIVE_REVENUE_CONTRACTS.UNISWAP_V3,
                    bwaeziAmountWei
                ).encodeABI()
            };

            const approveResult = await this.transactionManager.executeOptimizedTransaction(
                approveTxData,
                'BWAEZI_APPROVAL'
            );
            ...
            await this.transactionManager.delay(5000); // Wait for approval to confirm

            // 🎯 STEP 2: Execute BWAEZI → WETH swap via Uniswap V3
            logger.info(`🔄 Swapping ${bwaeziAmount} BWAEZI for WETH...`);
            ...
            // Calculate expected WETH output based on 1 BWAEZI = 100 USDC rate
            // Using current ETH price ~$3220 from your transaction history
            const ethPrice = 3220; // $ per ETH from your swap
            const usdcValue = bwaeziAmount * 100; // 1 BWAEZI = 100 USDC
            const expectedEth = usdcValue / ethPrice;
            const expectedEthWei = this.blockchain.web3.utils.toWei(expectedEth.toString(), 'ether');
            
            // Set minimum output with 5% slippage
            const amountOutMinimum = Math.floor(parseInt(expectedEthWei) * 0.95);

            const swapParams = {
                tokenIn: this.transactionManager.validateAddressSecurity(BWAEZI_TOKEN_CONFIG.CONTRACT_ADDRESS, 'BWAEZI_SWAP'),
                tokenOut: this.transactionManager.validateAddressSecurity(LIVE_REVENUE_CONTRACTS.WETH, 'BWAEZI_SWAP'),
                fee: 3000, // 0.3% pool fee
                recipient: this.transactionManager.account.address,
                deadline: Math.floor(Date.now() / 1000) + 1200, // 20 minutes
                amountIn: bwaeziAmountWei,
            ...
            const swapTxData = {
                to: LIVE_REVENUE_CONTRACTS.UNISWAP_V3,
                data: router.methods.exactInputSingle(swapParams).encodeABI()
            };

            const swapResult = await this.transactionManager.executeOptimizedTransaction(
                swapTxData,
                'BWAEZI_TO_WETH_SWAP'
            );

            if (swapResult.success && swapResult.optimized) {
                // Calculate revenue based on swap
                const revenue = usdcValue; // Revenue in USDC equivalent
                this.revenueGenerated += revenue;
                this.optimizedTrades++;
                this.bwaeziTrades++;
                this.totalProfit += revenue;
                
                logger.info(`✅ BWAEZI SWAP SUCCESS: ${bwaeziAmount} BWAEZI → ~${expectedEth.toFixed(6)} WETH`);
                logger.info(`💰 REVENUE: $${revenue.toFixed(2)} USDC equivalent`);
                
                return { 
                    success: true, 
                    revenue: revenue,
                    bwaeziSwapped: bwaeziAmount,
                    expectedWeth: expectedEth,
                    txHash: swapResult.txHash,
                    type: 'BWAEZI_TO_WETH',
                    profit: revenue,
                    optimized: true,
                    strategy: 'BWAEZI_PREMIUM_SWAP'
                };
            } else {
            ...
            console.error(`❌ BWAEZI to WETH swap failed: ${error.message}`);
            return { 
                success: false, 
                error: error.message, 
                optimized: false 
    ...
    // 🎯 OPTIMIZED: Manual BWAEZI swap control
    async executeManualBwaeziSwap(amount = 1) {
        try {
            const logger = getGlobalLogger('ManualBwaeziSwap');
            
            logger.info(`🎯 EXECUTING MANUAL BWAEZI SWAP: ${amount} tokens`);
            logger.info(`💸 EXPECTED VALUE: $${amount * 100} USDC equivalent`);
            
            const result = await this.executeBwaeziToWethSwap(amount);
            
            if (result.success) {
                logger.info(`✅ MANUAL SWAP SUCCESS: +$${result.revenue.toFixed(2)}`);
                
                // Update wallet balances after swap
                const [ethBalance, bwaeziBalance] = await Promise.all([
                    this.blockchain.getWalletBalance(this.transactionManager.account.address),
                    this.blockchain.getBwaeziTokenBalance(this.transactionManager.account.address)
                ]);
                
                logger.info(`💰 UPDATED BALANCES:`);
                logger.info(`   ETH: ${ethBalance}`);
                logger.info(`   BWAEZI: ${bwaeziBalance.balance}`);
                
            } else {
                logger.error(`❌ MANUAL SWAP FAILED: ${result.error}`);
            }
            
            return result;
            
        } catch (error) {
            console.error(`💥 MANUAL BWAEZI SWAP CRASHED: ${error.message}`);
            return { 
                success: false, 
                error: error.message 
            };
    ...
    registerLiveAgents() {
        ...
        // 🎯 ADD BWAEZI SWAP AGENT
        this.liveAgents.set('bwaezi-premium-swap', { 
            execute: async () => await this.executeBwaeziToWethSwap(1), // Swap 1 BWAEZI at a time
            weight: 0.35, // Higher weight for premium BWAEZI swaps
            cooldown: 60000, // 1 minute cooldown
            type: 'BWAEZI_PREMIUM',
            security: 'MAXIMUM_OPTIMIZED',
            revenue: 100.00 // $100 per BWAEZI token
        });

        console.log(`🎯 REGISTERED ${this.liveAgents.size} PREMIUM OPTIMIZED AGENTS INCLUDING BWAEZI SWAPS`);
