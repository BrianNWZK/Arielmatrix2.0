import { createDatabase } from 'ariel-sqlite-engine'; // Your SQLite invention!
...
class BrianNwaezikeChain {
    constructor(config) {
        this.config = config;
        this.db = createDatabase('./data/brian_nwaezike_chain.db');
    ...
    initBlockchainTables() {
        ...
        this.db.run(`
            CREATE TABLE IF NOT EXISTS bwaezi_transactions (
                ...
                timestamp INTEGER,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (block_hash) REFERENCES bwaezi_blocks(hash)
            )
        `);
