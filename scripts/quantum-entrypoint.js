...
const main = async () => {
    ...
    const entryPoints = ["arielsql_suite/server.js", "arielsql_suite/main.js", "server.js", "scripts/live-revenue-server.js"];
    let launched = false;
    for (const ep of entryPoints) {
        if (fs.existsSync(path.join('/app', ep))) {
            console.log(`🎯 Launching ${ep}`);
            // Use exec to replace the current process with the new one
            exec(`node /app/${ep}`, (error, stdout, stderr) => {
                if (error) {
                    console.error(`Failed to launch ${ep}:`, error);
                    return;
                }
                console.log(stdout);
