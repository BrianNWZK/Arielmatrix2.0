...
// Correct factory ABI from Balancer contract
const factoryAbi = [
...
async function createWeightedPool(name, symbol, tokenA, tokenB) {
  ...
  try {
    ...
    if (receipt.logs.length === 0) {
      ...
      // Try to check if pool was created by calling factory with static
      try {
        const staticResult = await factory.create.staticCallResult(
          name,
          symbol,
          token0,
          token1,
...
async function runSeeding() {
  ...
  if (!factoryOk) {
    console.log("❌ Factory check failed. Aborting.");
    return;
  }
