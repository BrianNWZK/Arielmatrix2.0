...
export class EnterpriseLogger {
    ...
    async deferDatabaseInitialization() {
        ...
        if (this.config.logToDatabase && this.db) {
            try {
                ...
                return this; // Return logger instance on success
            } catch (error) {
                this.logger.warn(`⚠️ Database logging deferred: ${error.message}`);
                // Don't retry aggressively - database might not be ready
                throw error; // Propagate failure for promise chain
            }
        }
        return this; // Return logger instance if DB logging is not enabled or DB is null
    }
    ...
    async setDatabase(database) {
        ...
        if (this.config.logToDatabase) {
            // Re-trigger deferred initialization with the new DB
            this.deferredDatabaseInit = this.deferDatabaseInitialization();
            await this.deferredDatabaseInit;
        }
    }
    ...
    async markDatabaseReady() {
        this.databaseReady = true;
        if (this.config.logToDatabase) {
            // This is mostly for external triggers, ensuring deferred init has run
            this.deferredDatabaseInit = this.deferDatabaseInitialization();
            await this.deferredDatabaseInit;
        }
    }
...
export function initializeGlobalLogger(serviceName = 'application', config = {}) {
    if (!globalLogger) {
        ...
        // FIX: Complete the promise assignment to allow other modules to wait for deferred DB init
        globalLoggerPromise = globalLogger.deferredDatabaseInit || Promise.resolve(globalLogger);
    }
    return globalLogger;
}

// FIX: Add getGlobalLogger for self-healing access (addresses the early access warnings)
/**
 * Safely returns the global logger instance, initializing a fallback if needed.
 * @param {string} serviceName - The name of the service requesting the logger.
 * @param {object} config - Configuration options for fallback initialization.
 * @returns {EnterpriseLogger} The global logger instance.
 */
export function getGlobalLogger(serviceName = 'application', config = {}) {
    if (!globalLogger) {
        // Fallback initialization uses a safer default configuration
        console.warn(`⚠️ CRITICAL: Global logger accessed before initialization. Performing self-healing fallback setup for service: ${serviceName}.`);
        return initializeGlobalLogger(serviceName, { ...config, logToDatabase: false, logToFile: true });
    }
    return globalLogger;
}

// FIX: Add database management utility (for main.js to link the DB instance)
/**
 * Enables database logging and links the logger to an initialized database.
