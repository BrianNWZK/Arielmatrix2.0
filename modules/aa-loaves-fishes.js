...
class UniversalDexAdapter {
  ...
  async _v2Quote(tokenIn, tokenOut, amountIn) {
    // Use this adapter's factory (Uniswap V2 vs Sushi V2)
...
class OracleAggregator {
  ...
  async v3TwapUSD(bwaezi, tokenUSD=LIVE.TOKENS.USDC){
    try {
      ...
      // Uniswap v3 tick gives price of token1/token0
      const price = Math.pow(1.0001, twapTick);
      const bwaeziIsToken0 = token0.toLowerCase() === bwaezi.toLowerCase();
      const usdIsToken1 = token1.toLowerCase() === tokenUSD.toLowerCase();
      const priceBWAEZIinUSDC = (bwaeziIsToken0 && usdIsToken1) ? price : (1 / price);
      return priceBWAEZIinUSDC;
    } catch { return null; }
  }
...
class StrategyEngine {
  ...
  async watchPeg() {
    ...
    const isWs =
      typeof (this.provider as any)._websocket !== 'undefined' ||
      (this.provider as any) instanceof (ethers as any).WebSocketProvider;
    ...
    if (isWs) {
      poolC.on('Swap', checkDeviation);
    } else {
      setInterval(checkDeviation, 5000);
    }
    return true;
  }
...
async function ensureScwDeployed(provider, aaSdk) {
  ...
  // No-op execute; AA SDK will include correct initCode via its own salt discovery when forceDeploy is true
...
// One-call orchestrator with SCW code-existence guard
async function runGenesisMicroseed(core) {
  try {
    ...
    const code = await core.provider.getCode(LIVE.SCW_ADDRESS);
    if (!code || code === '0x') {
      console.log('🌱 Skipping genesis — SCW not deployed yet');
      return { skipped: true, reason: 'scw_not_deployed' };
    }
